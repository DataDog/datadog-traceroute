...
// Traceroute runs an ICMP traceroute
func (i *ICMPv4) Traceroute() (*result.TracerouteRun, error) {
	...
	addr, conn, err := common.LocalAddrForHost(i.Target, 33434) // Use default traceroute port for address discovery
	if err != nil {
	...
	// Generate a random ICMP ID for this traceroute session
	i.icmpID = uint16(rand.Uint32() & 0xFFFF)
