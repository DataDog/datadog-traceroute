...
func validateLocalhostResults(t *testing.T, results *result.Results, protocol string) {
	...
	assert.Equal(t, protocol, results.Params.Protocol, "Protocol should match")
	assert.Equal(t, "127.0.0.1", results.Params.Hostname, "Hostname should be localhost")
...
func validatePublicEndpointResults(t *testing.T, results *result.Results, protocol, hostname string, port int) {
	...
	// Validate basic parameters
	assert.Equal(t, protocol, results.Params.Protocol, "Protocol should match")
	assert.Equal(t, hostname, results.Params.Hostname, "Hostname should match")
	assert.Equal(t, port, results.Params.Port, "Port should match")
