...
type sinkDarwin struct {
	...
	// isIPv6 indicates the socket is for IPv6. For IPv6 on macOS, the socket
	// is created lazily because IPPROTO_RAW doesn't support sendmsg with
	// ancillary data — we need the actual transport protocol (NextHeader)
	// which is only known from the packet buffer.
	isIPv6   bool
	ipv6Once sync.Once
	ipv6Err  error
}
